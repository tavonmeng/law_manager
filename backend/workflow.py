...
风格要求：措辞严谨简洁，采用法律意见书标准格式，每条结论直奔要害，不使用口语化表达。

请根据前序核查结论，提取所有存在合规风险的事项，并严格按以下格式逐条输出：

### [Issue编号]：[问题定性标题]

- **事实认定**：[用1-2句话简述客观数据事实，例如"经核查，目标公司签署劳动合同员工15人，养老保险实际缴费人数13人，2人未参保。"]
- **法律依据**：[援引具体法律条文，必须真实准确。格式示例："《中华人民共和国社会保险法》第五十八条规定：'用人单位应当自用工之日起三十日内为其职工向社会保险经办机构申请办理社会保险登记。'"]
- **投资人建议**：[站在投资方律师立场，给出针对本条Issue的具体、可执行的风控建议。例如：
  （1）要求目标公司提供未参保人员名单及未缴原因说明；
  （2）测算补缴金额及滞纳金敞口，纳入交易对价调整机制；
  （3）在SPA中将完成补缴列为交割前提条件（CP）；
  （4）由出售方/创始股东就该事项承担专项赔偿义务。]

注意事项：
1. 每条Issue的投资人建议必须针对该具体问题，不得泛泛而谈。
2. 如未发现实质性合规问题，输出"经核查，目标公司该模块暂未发现实质性合规风险"。
3. 建议项应编号列出，便于投决会逐项审议。
4. 仅输出有充分数据支撑的Issue。前序核查中标注为"材料不足"的项，不要作为Issue输出，直接在末尾汇总说明即可。"""
